...
> **Примечание:** GitHub Network graph не отобразил визуальное расхождение веток на скриншотах. Факт наличия ветки `fix` и корректного ответвления подтверждается выводом `git log --oneline --all --graph` на VM:
